...
// 注意：直接使用srv_send_msg=true时会占用主动消息频率，且多媒体文件不能复用，建议先上传，然后再使用消息发送类型进行发送
...
type MessageToCreate struct {
	...
	MsgID            string                    `json:"msg_id,omitempty"` // 要回复的消息id，为空是主动消息，公域机器人会异步审核，不为空是被动消息，公域机器人会校验语境
...
// Stream 流式消息信息
type Stream struct {
	State int32  `json:"state,omitempty"` // 流式消息状态 1正文生成中，10：正文生成结束， 11：引言消息生成中， 20：引导消息生成结束。
	ID    string `json:"id,omitempty"`    // 流式消息ID，流式消息第一条不用填写，第二条需要填写第一个分片返回的msgID.
	Index int32  `json:"index,omitempty"` // 流式消息的序号， 从0开始
...
type ActionButton struct {
	TemplateID   int32  `json:"template_id,omitempty"`   // 消息操作栏模板ID，与下面具体具体按钮二选一填写。待废弃字段！！！
...
type MessageReference struct {
	...
	IgnoreGetMessageError bool   `json:"ignore_get_message_error"` // 是否忽略获取消息失败错误
}
...
type Markdown struct {
	...
	CustomTemplateID string            `json:"custom_template_id"` // 自定义模版id
